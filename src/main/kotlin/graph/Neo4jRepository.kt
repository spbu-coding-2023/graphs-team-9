...
class Neo4jRepository(uri: String, user: String, password: String) : Closeable {
    ...
    fun putGraph(graph: Graph, graphName: String, isDirected: Boolean) {
    ...
    fun putAnalyzedGraphVM(graphVM: GraphVM, graphName: String, isDirected: Boolean) {
     ...
     private fun getGraphContent(emptyGraph: Graph, dbGraphName: String) {
        ...
        session.executeRead { tx ->
            ...
            tx.run(commands.toString()).stream().forEach { rec ->
                ...
                if (graph.verticesCount() > verticesCount) {
                    verticesCount++
                    verticesColors.add(Color(rec["red1"].asDouble().toFloat(), rec["green1"].asDouble().toFloat(), rec["blue1"].asDouble().toFloat()))
                ...
                if (!vertexValue2.isNull) {
                    ...
                    if (graph.verticesCount() > verticesCount) {
                        verticesCount++
                        verticesColors.add(Color(rec["red2"].asDouble().toFloat(), rec["green2"].asDouble().toFloat(), rec["blue2"].asDouble().toFloat()))
